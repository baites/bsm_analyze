...
double sffit(double * xar, double * par)
{
    double x = xar[0];
    double data = par[1] + par[2] * (x-100.0)*(x-100.0);
    double mc   = par[3] + par[4] * (x-100.0)*(x-100.0);

    double dfd1 = 1;
    double dfd2 = (x-100.0)*(x-100.0);

    double * p = par + 5;

    double s1, s2, sigma = 0;
...
double fitSigma(double * xar, double * par)
{
    double x = xar[0];
    double f = par[1] + par[2] * (x-100.0)*(x-100.0);
    double dfd1, dfd2;
...
void TriggerEfficienciesSF(TString datafile, TString mcfile, TString ref, TString id, TString variable, TString xtitle)
{
   ...
   for (Int_t i = 0; i < 4; ++i) dataparam[i+3] = datamatrix[i];
   ...
   for (Int_t i = 0; i < 4; ++i) mcparam[i+3] = mcmatrix[i];
